...
public class AutoIgnoreFeature extends AbstractAsunaTogglableFeature implements SimpleListener, IAsunaTickableFeature {
    ...
    public AutoIgnoreFeature() {
        super("AutoIgnore", AsunaCategory.CHAT);
    }
    ...
    private static void handleIgnoring() {
        for (HashMap.Entry<String, Integer> entry : spamViolationMap.entrySet()) {
            if (entry.getValue() > 40 && !ClientIgnoreFeature.ignorelist.contains(entry.getKey())) {
                ...
                AsunaMod.minecraft.player.sendMessage(new TextComponentString("\247" + "1--------------------------------------------------"));
                AsunaMod.minecraft.player.sendMessage(new TextComponentString("\247" + "4" + entry.getKey() + " " + "\247" + "6was automatically ignored."));
                AsunaMod.minecraft.player.sendMessage(new TextComponentString("\247" + "1--------------------------------------------------"));
