...
import com.sasha.asuna.mod.feature.impl.movement.YawLockFeature;
...
public class YawCommand extends SimpleCommand {
    ...
    public void onCommand() {
        if (this.getArguments() == null) {
            AsunaMod.logErr(false, "Arguments required! Try \"-help command yaw\"");
            return;
        }
        try {
            YawLockFeature.yawDegrees = Integer.parseInt(this.getArguments()[0]);
        } catch (Exception e) {
            AsunaMod.logErr(false, "Your argument must be an integer.");
        }
    }
}
